...
class Game_info {
    ...
    public:
        ...
        Game_info(std::string server_name, uint16_t size_x, uint16_t size_y, uint16_t game_length, std::map<player_id, player_t> _players, uint16_t bomb_timer, std::map<player_id, score> scores) :
            server_name{server_name}, size_x{size_x}, size_y{size_y}, game_length{game_length}, bomb_timer {bomb_timer}, scores{scores} {
                players = _players;
